...
- **Document that repositories using `transform --prov-from-git` must disable squash merging.** Provenance dates are read from the author date of each commit touching a concept file, and squashing replaces those commits with one dated at merge time, so creation dates are lost and silently rewritten by the next submission. The project setup guide previously recommended keeping squash merging enabled. Merge commits and rebase merging are both safe. [#371](https://github.com/nfdi4cat/voc4cat-tool/issues/371)
...
- Match the vocabulary name case-insensitively when reading its `allow_delete` setting. A vocabulary file with uppercase letters in its name silently fell back to forbidding deletion. [#359](https://github.com/nfdi4cat/voc4cat-tool/issues/359)
- Fail when `voc4cat docs` is asked for pyLODE documentation and pyLODE is missing. The command logged an error and returned successfully, so `merge.yml` and `publish.yml` would have gone green and published gh-pages without regenerated documentation if their pyLODE install step ever broke. [#375](https://github.com/nfdi4cat/voc4cat-tool/issues/375)
- Compare against the previous version of a vocabulary stored in split form. `check --ci-post` joined the split directory but discarded the result, so no previous version was found: every IRI counted as newly added and removals went unnoticed. Repositories keeping their vocabulary split - as voc4cat-template does - therefore never had `allow_delete` enforced. [#368](https://github.com/nfdi4cat/voc4cat-tool/issues/368)
