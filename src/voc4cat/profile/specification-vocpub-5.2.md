...
URI
[`https://linked.data.gov.au/def/vocpub/spec`](https://linked.data.gov.au/def/vocpub/spec)
...
Version IRI
<https://linked.data.gov.au/def/vocpub/spec/5.2>

Version Information
5.2 Implement Shape 2.1.2+3 using qualified shape pattern as this is supported by pySHACL

5.1 remove targetNode -> ConceptScheme shape, use schema:, not sdo:
...
Profile URI
[`https://linked.data.gov.au/def/vocpub`](https://linked.data.gov.au/def/vocpub)
...
-   <https://linked.data.gov.au/def/vocpub/validation>
...
between Concepts and relationships to Concepts in other vocabularies.
...
Codes</a></td>
<td>The Indigenous Data Network's vocabulary of the types of roles Agents
...
per <span style="color:darkred;">Requirement 2.1.10</span></td>
<td><a
...
style="color:darkred;">Requirement 2.1.11</span></td>
<td><a
...
indicated using the `schema:dateCreated` and `schema:dateModified` or
...
*MUST* have at least one creator, indicated using `schema:creator` or
`dcterms:creator` predicate and exactly one publisher, indicated using
`schema:publisher` or `dcterms:publisher`, all of which MUST be IRIs
indicating instances of `schema:Person`, or `schema:Organization`. A
...
`isorole:publisher` may be used instead of `schema:creator` &
`schema:publisher`, respectively
...
predicates: `skos:historyNote`, `schema:citation`, `prov:wasDerivedFrom`.
`dcterms:source` *MAY* be used instead of `schema:citation` and
...
`schema:citation` should be used to indicate the resource's IRI with the
...
status of the vocabulary *SHOULD* be given with the predicate
`prov:qualifiedDerivation` indicating a Blank Node that contains the
predicated `prov:entity`, to indicate the vocabulary derived from and
`prov:hadRole` to indicate the mode of derivation which *SHOULD* be
taken from the *Vocabulary Derivation Modes* vocabulary
...
theming of a vocabulary *SHOULD* be given using the `schema:keywords`
...
using the `schema:license` predicate preferentially indicating the IRI of a
license if in RDF form or a literal URL (datatype `xsd:anyURI`) if
online but not in RDF form. If the licence is expressed in text, a
...
`schema:copyrightHolder` predicate preferentially indicating the IRI of an
...
`isorole:rightsHolder` may be used instead of `schema:copyrightHolder`.
...
`skos:historyNote`, `schema:citation`, `prov:wasDerivedFrom`.
`dcterms:source` *MAY* be used instead of `schema:citation` and
`dcterms:provenance` *MAY* be used instead of `skos:historyNote` but the
schema.org and SKOS predicates are preferred.
...
indicated by at least one of the following predicates:
`skos:historyNote`, `schema:citation`, `dcterms:source` or
...
associated with a vocabulary *MUST* be typed as an `schema:Person` or
`schema:Organization`
...
give exactly one name with the `schema:name` predicate indicating a literal
text value
...
indicate either a `schema:url` (for organizations) or a `schema:email` (for
...
`schema:identifier` predicate with literal identifier values,
